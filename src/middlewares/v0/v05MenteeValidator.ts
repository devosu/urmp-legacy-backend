// ./src/middleware/v0/v05MenteeValidator.ts
//
// 5th step in the middleware chain, the Mentee user schema validator,
...
export default function v05MenteeValidator(
  ...
  try {
    ...
    if (userType === UserType.Enum.Mentee) {
      ...
      next(
        new InvalidSchemaError({
          ...
          details:
            "This error is caught by the mentee user schema validation middleware.",
        }),
      );
    }
    ...
    if (error instanceof ZodError) {
      next(
        new InvalidSchemaError({
          ...
          details:
            "This error is caught by the mentee user schema validation middleware through Zod, and pretty-printed by zod-validation-error.",
        }),
      );
    } else {
      next(error);
