// ./src/middleware/v0/v05NewSignupValidator.ts
//
// 5th step in the middleware chain, the NewSignup user schema validator,
    ...
    if (userType === UserType.Enum.NewSignup) {
      ...
      next(
        new InvalidSchemaError({
          ...
          details:
            "This error is caught by the new signup user schema validation middleware.",
        }),
      );
    }
    ...
    if (error instanceof ZodError) {
      next(
        new InvalidSchemaError({
          ...
          details:
            "This error is caught by the new signup user schema validation middleware through Zod, and pretty-printed by zod-validation-error.",
        }),
      );
    } else {
      next(error);
